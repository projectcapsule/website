...
If you run Capsule in [Strict Mode](/docs/operating/setup/installation/#strict-rbac), the controller must be allowed to grant the corresponding permissions to the `ServiceAccount` in all selected `Namespaces`. You can either grant the controller the `bind` verb on these `ClusterRoles` (see [Strict](#strict)) or aggregate the same `ClusterRoles` to the controller:
