...
| Behavior | Explanation |
|---|---|
| Only namespaced resources are evaluated | Metadata rules are intended for Tenant namespace resources. Cluster-scoped resources are not evaluated by the generic metadata webhook. |
...
| `kinds` must be set | Use `kinds: ["*"]` to match all kinds. |
...
| `kinds` | List of Kubernetes kinds to match. Use `"*"` to match all kinds. |
...
To match deployments across API versions, use `"*"`:
